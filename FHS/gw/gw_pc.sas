...
data temp;
  merge gw.pc(in=t rename=(pc=fid)) gw(drop=fid) gw.eig;
  by iid;
  if t;
run;
...
data gmr_vars(keep= name);
 ...
 file "/lustre/scr/v/i/victorw/gw/gwpct&sect..sas" ls= 2000 ;
...
run;
%include "/lustre/scr/v/i/victorw/gw/gwpct&sect..sas";
data gw.mx_gw_pc&sect.;
 set mx_all;
 snp=strip(snp);
run;

%sysexec rm /lustre/scr/v/i/victorw/gw/gwpct&sect..sas ;
