cd D:\AM\FHS550k\agcon\palst

usesas using pafreq_all, clear
...
bysort typef col: sum percent
ta col typef, sum(percent) 
ta col typef, sum(percent) nome nof
ta col typef, sum(percent) nost nof
/*
keep if typef=="sp"
keep pair col count percent
reshape wide count percent, i(pair) j(col)
egen count=rowtotal(count*)
egen percent=rowtotal(percent*)
sum count percent
*/
keep if col==4
