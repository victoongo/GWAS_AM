libname snp '/lustre/scr/y/i/yilanfu/aghrs/snplst';
libname agp '/lustre/scr/y/i/yilanfu/aghrs/plst';
...
data tempm;
  merge temp(in=t drop=ipid) snp.ag500km;
  by iid;
  if t;
run;
...
proc mixed data=tempr method=ml noitprint noclprint ratio covtest; 
  ...
  ods output CovParms=agrul.mx_agrul_p&sect.; 
run;
ods listing;
%end;
%MEND;
